...
def send_prompt_with_images_openai(story_images, character_images, instruction_text, seed, template_name, max_tokens):
        ...
        with ThreadPoolExecutor(max_workers=concurrency) as executor:
            futures = [
                executor.submit(
                    ...
                    seed, vwp_df, target_words_map, max_tokens,
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate stories using Llama-4-Scout model")
    parser.add_argument("--csv_file", type=str, default=None, help="Optional CSV with a story_id column. If omitted, story IDs are taken from the target source.")
    parser.add_argument("--output_dir", type=str, required=True, help="Base directory to save outputs")
    parser.add_argument("--template_dir", type=str, default=os.path.join(REPO_ROOT, "data", "prompts"), help="Directory containing prompt templates")
    parser.add_argument(
        "--template_name",
        type=str,
        required=True,
    ...
    parser.add_argument("--concurrency", type=int, default=1, help="Number of stories to send concurrently. Should be <= the server's --max-num-seqs.")
